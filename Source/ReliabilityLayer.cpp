...
namespace
{
	...
	// keep connected sessions alive even when ACK flow is broken.
	static const bool LEGACY_KEEP_CONNECTED_WITHOUT_ACK = true;

	void LogUnreadHexPreview(RakNet::BitStream *bitStream, int maxBytes = 32)
	{
		if (bitStream == 0 || SAMPRakNet::GetCore() == 0 || !SAMPRakNet::IsVerboseNetworkLogs())
			return;

		const int readOffset = bitStream->GetReadOffset();
		const int startByte = readOffset >> 3;
		const int bitOffset = readOffset & 7;
		const int totalBytes = bitStream->GetNumberOfBytesUsed();
		if (startByte >= totalBytes)
		{
			SAMPRakNet::GetCore()->printLn(
				"[CLIENT][rl-drop] unread hex preview empty readOffset=%d totalBytes=%d",
				readOffset,
				totalBytes);
			return;
		}

		const int available = totalBytes - startByte;
		const int bytesToDump = available < maxBytes ? available : maxBytes;
		const unsigned char *rawData = (const unsigned char*)bitStream->GetData();
		static const char hex[] = "0123456789ABCDEF";
		char preview[32 * 3 + 1];
		int writeOffset = 0;
		for (int i = 0; i < bytesToDump; ++i)
		{
			const unsigned char value = rawData[startByte + i];
			preview[writeOffset++] = hex[(value >> 4) & 0x0F];
			preview[writeOffset++] = hex[value & 0x0F];
			if (i + 1 < bytesToDump)
				preview[writeOffset++] = ' ';
		}
		preview[writeOffset] = '\0';

		SAMPRakNet::GetCore()->printLn(
			"[CLIENT][rl-drop] unread hex bitOffset=%d bytes=%d/%d preview=%s",
			bitOffset,
			bytesToDump,
			available,
			preview);
	...
	const InternalPacket* FindPendingResendPacket(const DataStructures::Queue<InternalPacket*>& resendQueue)
	{
		const unsigned int queueSize = resendQueue.Size();
		for (unsigned int i = 0; i < queueSize; ++i)
		{
			const InternalPacket* packet = resendQueue[i];
			if (packet && packet->nextActionTime != 0)
				return packet;
		}
		return 0;
	}
...
//	bool duplicatePacket;
	...
	const unsigned char *payloadBuffer = (const unsigned char*)buffer;
	int payloadLength = length;
	unsigned char decryptedBuffer[MAXIMUM_MTU_SIZE];
	unsigned char decryptInputBuffer[MAXIMUM_MTU_SIZE];
	bool usedReliabilityDecrypt = false;
	bool usedPlaintextFallback = false;
	...
	if (encryptor.IsKeySet())
	{
		...
		if (length > MAXIMUM_MTU_SIZE)
		{
			...
			{
				SAMPRakNet::GetCore()->printLn(
					"[CLIENT][rl-parse] inbound length exceeds MAXIMUM_MTU_SIZE len=%d max=%d",
					length,
					MAXIMUM_MTU_SIZE);
			}
			return false;
		}
		memcpy(decryptInputBuffer, buffer, length);
		const bool decryptOk = encryptor.Decrypt(
			decryptInputBuffer,
			length,
			decryptedBuffer,
			&decryptedLength);

		if (decryptOk && decryptedLength > 0)
		{
			payloadBuffer = decryptedBuffer;
			payloadLength = decryptedLength;
			usedReliabilityDecrypt = true;
		}
		else
		{
			...
			{
				SAMPRakNet::GetCore()->printLn(
					"[CLIENT][rl-parse] inbound DataBlock decrypt failed, falling back to plaintext parse len=%d",
					length);
			}
		}
	}

	statistics.bitsReceived += payloadLength * 8;
	statistics.packetsReceived++;

	if (SAMPRakNet::GetCore() && SAMPRakNet::IsVerboseNetworkLogs() && (usedReliabilityDecrypt || usedPlaintextFallback))
	{
		SAMPRakNet::GetCore()->printLn(
			"[CLIENT][rl-parse] inbound decode mode=%s srcLen=%d parseLen=%d",
			usedReliabilityDecrypt ? "decrypt" : "plaintext-fallback",
			length,
			payloadLength);
	}

	RakNet::BitStream socketData( (unsigned char*) payloadBuffer, payloadLength, false ); // Convert the incoming data to a bitstream for easy parsing
	...
	{
		SAMPRakNet::GetCore()->printLn(
			"[SERVER][ack-parse] peer=%s hasAcks=%d readOffset=%d unreadBits=%d parseLen=%d",
			playerId.ToString(true),
			hasAcks ? 1 : 0,
			socketData.GetReadOffset(),
			socketData.GetNumberOfUnreadBits(),
			payloadLength);
	}
	if (hasAcks)
	{
		MessageNumberType messageNumber;
		const int ackDeserializeStartOffset = socketData.GetReadOffset();
		if (incomingAcks.Deserialize(&socketData)==false)
		{
			if (SAMPRakNet::GetCore() && SAMPRakNet::IsVerboseNetworkLogs())
			{
				SAMPRakNet::GetCore()->printLn(
					"[SERVER][ack-parse] deserialize-failed peer=%s startOffset=%d endOffset=%d unreadBits=%d parseLen=%d",
					playerId.ToString(true),
					ackDeserializeStartOffset,
					socketData.GetReadOffset(),
					socketData.GetNumberOfUnreadBits(),
					payloadLength);
			}
		...
		if (SAMPRakNet::GetCore() && SAMPRakNet::IsVerboseNetworkLogs())
		{
			...
			{
				SAMPRakNet::GetCore()->printLn(
					"[SERVER][ack-range] from %s min=%u max=%u",
					playerId.ToString(true),
					(unsigned int)incomingAcks.ranges[i].minIndex,
					(unsigned int)incomingAcks.ranges[i].maxIndex);
			}
		}
		for (i=0; i<incomingAcks.ranges.Size();i++)
		{
			...
			if (messageHoleLimit > 0 && incomingAcks.ranges[i].maxIndex - incomingAcks.ranges[i].minIndex > messageHoleLimit)
			{
			...
			for (messageNumber=incomingAcks.ranges[i].minIndex; messageNumber >= incomingAcks.ranges[i].minIndex && messageNumber <= incomingAcks.ranges[i].maxIndex; messageNumber++)
			{
				...
				{
					SAMPRakNet::GetCore()->printLn(
						"[SERVER][ack-in] from %s msg=%u result=%s resendSize=%u",
						playerId.ToString(true),
						(unsigned int)messageNumber,
						ackedHistogramCounter==(unsigned)-1 ? "miss" : "hit",
						(unsigned int)resendList.Size());
				}
...
//				internalPacketPool.ReleasePointer( internalPacket );
			...
			// Detect both sustained over-limit traffic (perSecond) and short spikes within the current 1s window (perFrame).
			if (acksLimit > 0 && (statistics.perSecondAcksLimitCounter > acksLimit || statistics.perFrameAcksLimitCounter > acksLimit))
			{
				const char * ipPort = playerId.ToString(true);
				SAMPRakNet::GetCore()->logLn(
					LogLevel::Warning,
					"client exceeded 'ackslimit' %s (frame=%d/sec=%d) Limit: %d/sec",
					ipPort,
					statistics.perFrameAcksLimitCounter,
					statistics.perSecondAcksLimitCounter,
					acksLimit);
				incomingAcks.Clear();
				shouldBanPeer = true;
				return 1;
	...
	if (internalPacket==0)
		...
		{
			if (SAMPRakNet::GetCore() && internalPacket->data && internalPacket->dataBitLength >= 8)
			{
				const unsigned int packetId = (unsigned int)(unsigned char)internalPacket->data[0];
				if (SAMPRakNet::ShouldLogSyncOrRPCPacket((uint8_t)packetId))
				{
					SAMPRakNet::GetCore()->printLn("[CLIENT] packet id=%u bits=%u from %s",
						packetId,
						(unsigned int)internalPacket->dataBitLength,
						playerId.ToString(true));
				}
			}
...

		{
			...
			// Detect both sustained over-limit traffic (perSecond) and short spikes within the current 1s window (perFrame).
			if (messagesLimit > 0 && (statistics.perSecondMessagesLimitCounter > messagesLimit || statistics.perFrameMessagesLimitCounter > messagesLimit))
			{
				const char* ipPort = playerId.ToString(true);
				SAMPRakNet::GetCore()->logLn(
					LogLevel::Warning,
					"client exceeded 'messageslimit' %s (frame=%d/sec=%d) Limit: %d/sec",
					ipPort,
					statistics.perFrameMessagesLimitCounter,
					statistics.perSecondMessagesLimitCounter,
					messagesLimit);
				incomingAcks.Clear();
				shouldBanPeer = true;
				return 1;
			...
			if ( internalPacket->reliability == RELIABLE_ORDERED )
			{
				...
				//	RakAssert(waitingForOrderedPacketReadIndex[ internalPacket->orderingChannel ] < internalPacket->orderingIndex);
					...
					if (messageHoleLimit > 0 && statistics.orderedMessagesOutOfOrder > messageHoleLimit)
					{
						const char* ipPort = playerId.ToString(true);
...
bool ReliabilityLayer::Send( char *data, int numberOfBitsToSend, PacketPriority priority, PacketReliability reliability, unsigned char orderingChannel, bool makeDataCopy, int MTUSize, RakNetTimeNS currentTime )
{
	...
	// 1-byte keepalive/control packet (id 34 / 0x22) can get stuck in resend loops on some mobile routes.
	// It is safe to send it unreliably to prevent ACK-resend oscillation.
	if (numberOfBitsToSend == 8 && data != 0 && (unsigned char)data[0] == 34 &&
...
void ReliabilityLayer::Update( SOCKET s, PlayerID playerId, int MTUSize, RakNetTimeNS time, DataStructures::List<PluginInterface*> &messageHandlerList )
{
	...
	// Compatibility guard: do not drop by ACK timeout while we still receive traffic from peer.
	const bool ackTimedOut =
		resendList.IsEmpty()==false &&
		time > lastAckTime &&
		lastAckTime &&
		time - lastAckTime > (RakNetTimeNS)timeoutTime*1000;
	const bool recvTimedOut =
		time > lastReceiveTime &&
		time - lastReceiveTime > (RakNetTimeNS)timeoutTime*1000;
	const InternalPacket* pendingResendPacket = FindPendingResendPacket(resendQueue);

	if (ackTimedOut && recvTimedOut == false)
	{
		// We still receive packets from peer but a reliable resend is not ACKed.
		// Keep the session alive and wait for either an ACK or true receive timeout.
		if (SAMPRakNet::GetCore())
		{
			const unsigned int packetId =
				(pendingResendPacket && pendingResendPacket->data && pendingResendPacket->dataBitLength >= 8)
				? (unsigned int)(unsigned char)pendingResendPacket->data[0]
				: 0xFFFFFFFFu;
			const unsigned int resendMsg =
				pendingResendPacket ? (unsigned int)pendingResendPacket->messageNumber : 0xFFFFFFFFu;
			const unsigned int resendRel =
				pendingResendPacket ? (unsigned int)pendingResendPacket->reliability : 0xFFFFFFFFu;
			const unsigned int resendAgeMs =
				pendingResendPacket && time > pendingResendPacket->creationTime
				? (unsigned int)((time - pendingResendPacket->creationTime) / 1000)
				: 0u;
			const unsigned int recvAgoMs =
				time > lastReceiveTime ? (unsigned int)((time - lastReceiveTime) / 1000) : 0u;

			SAMPRakNet::GetCore()->printLn(
				"[SERVER][ack-stall] peer=%s msg=%u rel=%u pktId=%u resendAgeMs=%u recvAgoMs=%u timeoutMs=%u resendSize=%u",
				playerId.ToString(true),
				resendMsg,
				resendRel,
				packetId,
		...
		// Re-arm ack timer so we don't treat this as immediate dead-connection condition
		// while inbound traffic is still active.
		lastAckTime = time;
	}

	if (ackTimedOut && recvTimedOut)
	{
		...
		if (SAMPRakNet::GetCore())
		{
			...
			const unsigned int resendRel =
				pendingResendPacket ? (unsigned int)pendingResendPacket->reliability : 0xFFFFFFFFu;
			const unsigned int resendBits =
				pendingResendPacket ? (unsigned int)pendingResendPacket->dataBitLength : 0u;

			SAMPRakNet::GetCore()->printLn(
				"[SERVER][dead-connection] peer=%s now=%u lastAck=%u lastRecv=%u timeoutMs=%u resendSize=%u stuckMsg=%u stuckRel=%u stuckPkt=%u stuckBits=%u",
				playerId.ToString(true),
				(unsigned int)(time/1000),
				(unsigned int)(lastAckTime/1000),
				(unsigned int)(lastReceiveTime/1000),
				(unsigned int)timeoutTime,
				(unsigned int)resendList.Size(),
				resendMsg,
				resendRel,
		...
		if (LEGACY_KEEP_CONNECTED_WITHOUT_ACK == false)
		{
			deadConnection = true;
			return;
		}
		// Keep processing in compatibility mode.
		lastAckTime = time;
	}

	// Compatibility mode:
	// don't hard-drop purely on receive idle when there are no pending reliable resends.
	// Legacy clients behind NAT can go silent for long periods and still be valid.
	...
	*reliableDataSent = false;
	...
	if (time > nextAckTime)
	{
		if (acknowlegements.Size()>0)
		{
			...
			if (acknowlegements.Size()==0)
				// Match legacy client behavior: once pending ACK ranges are fully serialized,
				// allow immediate ACK scheduling on the next update tick.
				nextAckTime=time;
			else
			{
			//	printf("Ack full\n");
...
//		internalPacketPool.ReleasePointer( internalPacket );
	...
	else
	{
		...
		{
			SAMPRakNet::GetCore()->printLn(
				"[SERVER][ack-miss] msg=%u resendSize=%u",
				(unsigned int)messageNumber,
				(unsigned int)resendList.Size());
		}
	}
...
int ReliabilityLayer::GetBitStreamHeaderLength( const InternalPacket *const internalPacket )
{
	...
	{
		// Client RE (sub_630544/sub_6320B4): orderingChannel is serialized in 4 bits.
...
int ReliabilityLayer::WriteToBitStreamFromInternalPacket( RakNet::BitStream *bitStream, const InternalPacket *const internalPacket )
{
	...
	if ( internalPacket->reliability == UNRELIABLE_SEQUENCED || internalPacket->reliability == RELIABLE_SEQUENCED || internalPacket->reliability == RELIABLE_ORDERED )
	{
		// orderingIndex first, then orderingChannel in 4 bits.
...
InternalPacket* ReliabilityLayer::CreateInternalPacketFromBitStream( RakNet::BitStream *bitStream, RakNetTimeNS time )
{
	...
	InternalPacket* internalPacket;
	auto logDrop = [&](const char* reason, bool dumpUnreadHex = false)
	{
		if (SAMPRakNet::GetCore() && SAMPRakNet::IsVerboseNetworkLogs())
		{
			SAMPRakNet::GetCore()->printLn("[CLIENT][rl-drop] %s unreadBits=%d readOffset=%d",
				reason, bitStream->GetNumberOfUnreadBits(), bitStream->GetReadOffset());
			if (dumpUnreadHex)
				LogUnreadHexPreview(bitStream);
		}
	};
	...
	{
		logDrop("read messageNumber failed");
		internalPacketPool.ReleasePointer( internalPacket );
		return 0;
	}
	...
	if ( bitStreamSucceeded == false )
	{
		logDrop("post messageNumber validation failed");
		internalPacketPool.ReleasePointer( internalPacket );
		return 0;
	}
	...
	const int reliabilityBits = 3;
	const int orderingChannelBits = 4;
	bitStreamSucceeded = bitStream->ReadBits( ( unsigned char* ) ( &( reliability ) ), reliabilityBits );
	...
	if ( bitStreamSucceeded == false )
	{
		logDrop("read reliability(3 bits) failed");
		internalPacketPool.ReleasePointer( internalPacket );
		return 0;
	}
	if (reliability > (unsigned char)RELIABLE_SEQUENCED)
	{
		logDrop("invalid reliability value");
		internalPacketPool.ReleasePointer( internalPacket );
		return 0;
	}
	...
	if ( internalPacket->reliability == UNRELIABLE_SEQUENCED || internalPacket->reliability == RELIABLE_SEQUENCED || internalPacket->reliability == RELIABLE_ORDERED )
	{
		// Compatibility: client serializes orderingIndex first (16 bits), then orderingChannel (4 bits).
		unsigned short ordIdx16 = 0;
		bitStreamSucceeded = bitStream->ReadBits( ( unsigned char* ) &ordIdx16, 16 );
		internalPacket->orderingIndex = (OrderingIndexType)ordIdx16;
		...
		if ( bitStreamSucceeded == false )
		{
			logDrop("read orderingIndex failed");
			internalPacketPool.ReleasePointer( internalPacket );
			return 0;
		}

		// ordering channel encoded in 4 bits
		bitStreamSucceeded = bitStream->ReadBits( ( unsigned char* ) & ( internalPacket->orderingChannel ), orderingChannelBits );
#ifdef _DEBUG
		// 10/08/05 - Disabled assert since this hits from offline packets
		//RakAssert( bitStreamSucceeded );
#endif
		...
		if ( bitStreamSucceeded == false )
		{
			logDrop("read orderingChannel(4 bits) failed", true);
			internalPacketPool.ReleasePointer( internalPacket );
			return 0;
		}
	}
	...
	if ( bitStreamSucceeded == false )
	{
		logDrop("read isSplitPacket failed");
		internalPacketPool.ReleasePointer( internalPacket );
		return 0;
	}
	...
	if ( isSplitPacket )
	{
		...
		if ( bitStreamSucceeded == false )
		{
			logDrop("read splitPacketId failed", true);
			internalPacketPool.ReleasePointer( internalPacket );
			return 0;
		}
		...
		if ( bitStreamSucceeded == false )
		{
			logDrop("read splitPacketIndex failed", true);
			internalPacketPool.ReleasePointer( internalPacket );
			return 0;
		}
		...
		if ( bitStreamSucceeded == false )
		{
			logDrop("read splitPacketCount failed", true);
			internalPacketPool.ReleasePointer( internalPacket );
			return 0;
		}
		if (internalPacket->splitPacketCount == 0 || internalPacket->splitPacketIndex >= internalPacket->splitPacketCount)
		{
			logDrop("split packet dropped by policy", true);
			internalPacketPool.ReleasePointer( internalPacket );
			return 0;
		}
	...
	const int lengthReadOffset = bitStream->GetReadOffset();
	bool usedRawLengthFallback = false;
	...
	if ( bitStreamSucceeded == false )
	{
		// Compatibility fallback: some clients write payload length as raw 16 bits.
		bitStream->SetReadOffset(lengthReadOffset);
		bitStreamSucceeded = bitStream->Read(length);
		usedRawLengthFallback = bitStreamSucceeded;
		if ( bitStreamSucceeded == false )
		{
			logDrop("read payload length failed (compressed+raw16)");
			internalPacketPool.ReleasePointer( internalPacket );
			return 0;
		}
	}
	...
	{
		// If compressed length was parsed but produced nonsense, retry as raw 16-bit length.
		if (usedRawLengthFallback == false)
		{
			bitStream->SetReadOffset(lengthReadOffset);
			if (bitStream->Read(length))
			{
				internalPacket->dataBitLength = length;
				usedRawLengthFallback = true;
			}
		}

		if ( ! ( internalPacket->dataBitLength > 0 && BITS_TO_BYTES( internalPacket->dataBitLength ) < MAXIMUM_MTU_SIZE ) )
		{
			logDrop("invalid payload bitLength range");
	...
	if (usedRawLengthFallback && SAMPRakNet::GetCore())
	{
		SAMPRakNet::GetCore()->printLn(
			"[CLIENT][rl-parse] payload length read via raw16 fallback msg=%u rel=%u bitLen=%u readOffset=%d",
			(unsigned int)internalPacket->messageNumber,
			(unsigned int)internalPacket->reliability,
			(unsigned int)internalPacket->dataBitLength,
			bitStream->GetReadOffset());
	}
	if (SAMPRakNet::GetCore() && SAMPRakNet::IsVerboseNetworkLogs())
	{
		const bool hasOrdering = internalPacket->reliability == UNRELIABLE_SEQUENCED || internalPacket->reliability == RELIABLE_SEQUENCED || internalPacket->reliability == RELIABLE_ORDERED;
		SAMPRakNet::GetCore()->printLn(
			"[CLIENT][rl-parse] header msg=%u rel=%u ordCh=%u ordIdx=%u split=%u splitId=%u splitIdx=%u splitCnt=%u payloadBits=%u readOffset=%d unreadBits=%d",
			(unsigned int)internalPacket->messageNumber,
			(unsigned int)internalPacket->reliability,
			(unsigned int)(hasOrdering ? internalPacket->orderingChannel : 255),
			(unsigned int)(hasOrdering ? internalPacket->orderingIndex : 0),
			(unsigned int)(isSplitPacket ? 1 : 0),
			(unsigned int)internalPacket->splitPacketId,
			(unsigned int)internalPacket->splitPacketIndex,
			(unsigned int)internalPacket->splitPacketCount,
			(unsigned int)internalPacket->dataBitLength,
			bitStream->GetReadOffset(),
			bitStream->GetNumberOfUnreadBits());
	}

	const int payloadBytes = BITS_TO_BYTES( internalPacket->dataBitLength );
	if (SAMPRakNet::GetCore() && bitStream->GetNumberOfUnreadBits() < payloadBytes * 8)
	{
		SAMPRakNet::GetCore()->printLn(
			"[CLIENT][rl-parse] truncated before payload msg=%u rel=%u split=%u splitId=%u splitIdx=%u splitCnt=%u payloadBytes=%d unreadBits=%d readOffset=%d",
			(unsigned int)internalPacket->messageNumber,
			(unsigned int)internalPacket->reliability,
			(unsigned int)(isSplitPacket ? 1 : 0),
			(unsigned int)internalPacket->splitPacketId,
	...
	internalPacket->data = new unsigned char [ payloadBytes ];
	//printf("Allocating %i\n",  internalPacket->data);

	// Set the last byte to 0 so if ReadBits does not read a multiple of 8 the last bits are 0'ed out
	internalPacket->data[ payloadBytes - 1 ] = 0;

	const int payloadReadOffsetBeforeAlign = bitStream->GetReadOffset();
	if ((payloadReadOffsetBeforeAlign & 7) != 0)
	{
		bitStream->AlignReadToByteBoundary();
		if (SAMPRakNet::GetCore() && SAMPRakNet::IsVerboseNetworkLogs())
		{
			SAMPRakNet::GetCore()->printLn(
				"[CLIENT][rl-parse] aligned payload readOffset %d -> %d",
				payloadReadOffsetBeforeAlign,
				bitStream->GetReadOffset());
		}
	}

	// Read the data the packet holds
	bitStreamSucceeded = bitStream->ReadAlignedBytes( ( unsigned char* ) internalPacket->data, payloadBytes );
	...
	if ( bitStreamSucceeded == false )
	{
		...
		{
			SAMPRakNet::GetCore()->printLn(
				"[CLIENT][rl-parse] payload read failed msg=%u rel=%u split=%u splitId=%u splitIdx=%u splitCnt=%u payloadBytes=%d unreadBits=%d readOffset=%d",
				(unsigned int)internalPacket->messageNumber,
				(unsigned int)internalPacket->reliability,
				(unsigned int)(isSplitPacket ? 1 : 0),
				(unsigned int)internalPacket->splitPacketId,
		...
		logDrop("read aligned payload bytes failed");
		if (internalPacket->data)
		{
			delete [] internalPacket->data;
		}
...
InternalPacket * ReliabilityLayer::BuildPacketFromSplitPacketList( SplitPacketIdType splitPacketId, RakNetTimeNS time )
{
	...
	if (splitPacketChannelList[i]->splitPacketList.Size()==splitPacketChannelList[i]->splitPacketList[0]->splitPacketCount)
	{
		...
		if (SAMPRakNet::GetCore() && internalPacket->data && internalPacket->dataBitLength >= 8)
		{
			const unsigned int packetId = (unsigned int)(unsigned char)internalPacket->data[0];
			SAMPRakNet::GetCore()->printLn(
				"[CLIENT][split-rebuild] id=%u len=%u bits=%u splitId=%u parts=%u",
				packetId,
				(unsigned int)BITS_TO_BYTES(internalPacket->dataBitLength),
				(unsigned int)internalPacket->dataBitLength,
				(unsigned int)splitPacketId,
				(unsigned int)splitPacketChannelList[i]->splitPacketList[0]->splitPacketCount);
		}
...
bool ReliabilityLayer::HasUnacknowledgedMessage(MessageNumberType messageNumber) const
{
	InternalPacket *out = 0;
	return resendList.Get(messageNumber, out);
}
