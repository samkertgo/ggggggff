...
namespace RakNet
{
	namespace DataStructures
	{
		...
		{
			ranges.Clear();
			const int startReadOffset = out->GetReadOffset();
			unsigned short count = 0;
			bool usedFixed16Count = false;

			if (out->ReadCompressed(count) == false)
			{
				out->SetReadOffset(startReadOffset);
				if (out->Read(count) == false)
					return false;
				usedFixed16Count = true;
			}
			...
			for (i=0; i < count; i++)
			{
				if (out->Read(maxEqualToMin)==false)
				{
					// Compatibility fallback for peers that write fixed 16-bit count instead of compressed count.
					if (usedFixed16Count)
						return false;
					out->SetReadOffset(startReadOffset);
					ranges.Clear();
