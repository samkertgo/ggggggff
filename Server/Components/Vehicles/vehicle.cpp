...
void Vehicle::streamInForPlayer(IPlayer& player)
{
	...
	setPos.VehicleID = poolID;
	setPos.position = pos;
	PacketHelper::send(setPos, player);
	...
	// Repeat position after all stream-in RPCs in case client ignores early set-pos before full entity init.
	NetCode::RPC::SetVehiclePosition setPosLate;
	setPosLate.VehicleID = poolID;
	setPosLate.position = pos;
	PacketHelper::send(setPosLate, player);
