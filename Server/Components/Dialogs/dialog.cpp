...
private:
	...
	struct DialogResponseHandler : public SingleNetworkInEventHandler
	{
		...
		bool onReceive(IPlayer& peer, NetworkBitStream& bs) override
		{
			...
			if (data->style_ == DialogStyle_PASSWORD || data->style_ == DialogStyle_INPUT || data->style_ == DialogStyle_MSGBOX)
			{
				// Some clients/mods send 0 for non-list dialog styles instead of -1.
				// Keep compatibility and normalize to -1 so script handlers behave consistently.
				if (sendDialogResponse.ListItem != -1 && sendDialogResponse.ListItem != 0)
				{
					return false;
				}
				sendDialogResponse.ListItem = -1;
			}
